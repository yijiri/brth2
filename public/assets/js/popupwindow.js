...
(function($) {
    ...
    window.$mypop.prototype = {
        ...
        pop1: function( c, opt ) {
            ...
            //create popup window.
            var win = this.createMyWindow( width, height );
        ...
        createMyWindow: function( w, h ) {
           if( w != 'auto' && String( w ).match( /(px|em|%|pt)/i ) == null ) {
            ...
            // iziri versionが取得できなくてエラーになる。今回はIE6は対象ではないのででコメントアウト。
...
/**
* [実行中]画面を閉じて[結果表示]画面を表示
* @param url							URL
* @param disptitle						ツールバーのタイトル
*/
function closeWaitingPop(url, disptitle){
	document.body.style.cursor = 'auto';
	waitingPop.close();
	msgPop = new $mypop(url, { type: 'iframe', title: disptitle, width: 550, height: 250,windowmode:true, resize: true ,modal:true, close:false ,windowmode:false} );
}


/**
 * 実行エラーになったときに、[実行中]画面を閉じる
 * @param url		呼び出し元画面(再表示したりするかもしれないので念のため取得しておく)
 */
function closeWaitingPopForError(url){
	document.body.style.cursor = 'auto';
	waitingPop.close();
}
...
/**
 * APIを実行する画面をポップアップで表示
 * @param url			URL
 * @param disptitle		ツールバーのタイトル
 */
function executeApi(url, disptitle)
{
	waitingPop = new $mypop(url, { type: 'iframe', title: disptitle, width: 550, height: 250,windowmode:true, resize: true ,modal:true, close:false ,windowmode:false} );
}
