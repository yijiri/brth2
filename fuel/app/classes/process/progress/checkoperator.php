...
/**
 * 区分：ネットワーク管理
 * 概要：オペレータで設定されていないかチェック
 */
class process_network_checkoperator extends process_detailbase {
	 ...
	 */
	public function process_network_checkoperator($b_db_flg = true) {
		// 親のコンストラクタをコール
	...
	{
		$s_sql = '';
		$s_sql .= 'SELECT COUNT(*) AS CNT FROM COMPANY_MST MST ';
		$s_sql .= 'INNER JOIN OPERATOR_NETWORK_REL REL1 ';
		$s_sql .= 'ON MST.COMPANY_ID = REL1.COMPANY_ID ';
		$s_sql .= 'WHERE  NETWORK_ID = :NETWORK_ID ';
	...
	{
		Session::set(SESSION_NETWORK_CHECK_OPERATOR, $v_res_base);
	}
